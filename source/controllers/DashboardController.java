...
public class DashboardController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        //TODO Bassam each auction in currently selling doesn't seem to have the bids inside it
    ...
    private void initLineChart(List<Auction> currentlySelling, List<Auction> soldAuctions) {
        ...
        for (Auction auction : currentlySelling) {
            System.out.println("num of bidders on " + auction.getArtwork().getTitle() + " " + auction.getBidList().size());

//            System.out.println(Util.getAuctionByAuctionID(auction.getAuctionID()).getBidList().size());
...
//				System.out.println(bid.getBidderUsername());
    ...
    private void initBarChart(List<Auction> boughtAuctions, List<Auction> soldAuctions) {
        ...
        for (Auction elem : soldAuctions) {
            if (elem.getArtwork() instanceof Painting) {
                moneyEarnedPaintings = moneyEarnedPaintings + (int) (double) elem.getBidList().get(elem.getBidList()
                        .size() - 1).getBidAmount();
            } else {
